...
import { ChevronRight, ChevronDown, Menu, X, ArrowUpRight, Mail, MapPin, Clock, Shield, Zap, MessageCircle, FileText, Building2, Landmark, ScrollText, Store, Megaphone, ArrowRight, CheckCircle2, AlertTriangle, ChevronLeft, Star, Users, Globe, Smartphone, ArrowDown, Send, Search, Upload, CircleDollarSign, BadgeCheck, ShieldAlert, Timer, Truck, Lock, XCircle, Ban, Scale, Gavel, FileWarning, Receipt, Siren, Package, Eye } from "lucide-react";
...
// Servicios visibles en el menu/home. "permisos" se oculta: publicidad, mesas y sillas
// y ReBA son parte de Habilitaciones y se gestionan en habilitacioneslaplata.com.
// La data del bloque se conserva; /permisos-y-publicidad redirige (301) a HAB via vercel.json.
// Orden del menu (izq->der): ARBA y Catastro, Actas e Inscripciones, Habilitaciones, Registrales.
const MENU_ORDEN = ["arba", "actas", "registrales", "habilitaciones"];
const MENU_SERVICES = MENU_ORDEN.map(id => SERVICES.find(s => s.id === id)).filter(Boolean);

const GOOGLE_REVIEWS_URL = "https://search.google.com/local/reviews?placeid=ChIJC_1VIJTnopURA3KooOBAkiE";
const REVIEWS = [
  { text: "Excelente servicio! Lo contacté a través de un teléfono que me brindó el Colegio de Abogados de Trelew. Respondió enseguida. Cumplió con lo acordado y, ante un inconveniente en el registro, lo resolvió sin problemas.", author: "Dra. Gabriela Ferrari", rubro: "Abogada", stars: 5 },
...
function CookieBanner({ onPrivacy }) {
  ...
  const [visible, setVisible] = useState(() => {
    try {
      const raw = localStorage.getItem("glp_cookies_consent");
      if (!raw) return true;
      let ts = 0;
      try { ts = JSON.parse(raw).ts || 0; } catch { ts = 0; }
      return !ts || Date.now() - ts > SEIS_MESES_MS;
    } catch { return true; }
  });
  if (!visible) return null;
  const guardar = (valor) => {
    try { localStorage.setItem("glp_cookies_consent", JSON.stringify({ valor, ts: Date.now() })); } catch {}
    setVisible(false);
  };
  const accept = () => guardar("aceptado");
  const reject = () => guardar("rechazado");
  return <div style={{ position: "fixed", bottom: 0, left: 0, right: 0, zIndex: 1100, background: "rgba(45,55,72,0.97)", backdropFilter: "blur(12px)", borderTop: "1px solid rgba(255,255,255,0.1)", padding: "18px 24px", animation: "cookieSlide 0.4s ease-out" }}>
    <div style={{ maxWidth: 1200, margin: "0 auto", display: "flex", alignItems: "center", justifyContent: "space-between", gap: 20, flexWrap: "wrap" }}>
      <p style={{ fontFamily: "'Inter',sans-serif", fontSize: 12.5, color: "rgba(241,243,245,0.85)", margin: 0, flex: 1, minWidth: 280, lineHeight: 1.45 }}>
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 10, flexShrink: 0, flexWrap: "wrap" }}>
        <button onClick={accept} style={{ background: "#F4A261", color: "#1D3557", border: "none", borderRadius: 6, padding: "7px 16px", fontFamily: "'Poppins',sans-serif", fontWeight: 600, fontSize: 12, letterSpacing: "1px", textTransform: "uppercase", cursor: "pointer", transition: "background 0.2s" }} onMouseEnter={e => e.currentTarget.style.background = "#E08A48"} onMouseLeave={e => e.currentTarget.style.background = "#F4A261"}>Aceptar</button>
        <button onClick={reject} style={{ background: "transparent", color: "rgba(241,243,245,0.6)", border: "1px solid rgba(255,255,255,0.15)", borderRadius: 6, padding: "7px 16px", fontFamily: "'Poppins',sans-serif", fontWeight: 600, fontSize: 12, letterSpacing: "1px", textTransform: "uppercase", cursor: "pointer", transition: "all 0.2s" }} onMouseEnter={e => { e.currentTarget.style.borderColor = "rgba(255,255,255,0.3)"; e.currentTarget.style.color = "#F1F3F5"; }} onMouseLeave={e => { e.currentTarget.style.borderColor = "rgba(255,255,255,0.15)"; e.currentTarget.style.color = "rgba(241,243,245,0.6)"; }}>Rechazar</button>
        <button onClick={onPrivacy} style={{ background: "none", border: "none", color: "#F4A261", fontFamily: "'Inter',sans-serif", fontSize: 12, cursor: "pointer", textDecoration: "underline", padding: "7px 4px" }}>Política de privacidad</button>
      </div>
    </div>
  </div>;
}
...
function Btn({ children, v = "primary", href, onClick, style: s = {}, sm, full }) {
  const b = { display: "inline-flex", alignItems: "center", justifyContent: "center", gap: 8, border: "none", cursor: "pointer", fontFamily: "'Poppins',sans-serif", fontWeight: 700, fontSize: sm ? 13 : 15, letterSpacing: "1px", textTransform: "uppercase", textDecoration: "none", borderRadius: 8, padding: sm ? "10px 18px" : "15px 30px", transition: "all 0.25s", width: full ? "100%" : "auto", ...s };
  const vs = { primary: { background: "#F4A261", color: "#1D3557", boxShadow: "0 4px 16px rgba(244,162,97,0.35)" }, outline: { background: "transparent", color: "#1D3557", border: "2px solid #1D3557" }, outlineW: { background: "transparent", color: "#fff", border: "2px solid rgba(255,255,255,0.4)" }, ghost: { background: "transparent", color: "#3B6B8A" }, green: { background: "#25D366", color: "#fff", boxShadow: "0 4px 16px rgba(37,211,102,0.35)" } };
...
function Card({ children, style: s = {}, onClick, hover = true }) { const [h, sh] = useState(false); return <div onClick={onClick} onMouseEnter={() => sh(true)} onMouseLeave={() => sh(false)} style={{ background: "#fff", borderRadius: 12, padding: 28, boxShadow: h && hover ? "0 12px 40px rgba(29,53,87,0.12)" : "0 2px 12px rgba(29,53,87,0.06)", transition: "all 0.25s", cursor: onClick ? "pointer" : "default", transform: h && hover && onClick ? "translateY(-3px)" : "none", ...s, outline: h && hover ? "1px solid #1D3557" : "1px solid transparent", outlineOffset: "-1px" }}>{children}</div>; }
const ST = ({ badge, title, sub, light, center, badgeColor, num }) => { const kc = badgeColor || (light ? "#F4A261" : "#1D3557"); return <div style={{ marginBottom: 64, maxWidth: center ? 900 : 700, textAlign: center ? "center" : "left", margin: center ? "0 auto 48px" : undefined }}>{badge && <p style={{ display: "flex", alignItems: "center", gap: 12, fontFamily: "'Inter',sans-serif", fontSize: 11, fontWeight: 600, letterSpacing: "0.18em", textTransform: "uppercase", color: kc, margin: "0 0 18px", justifyContent: center ? "center" : "flex-start" }}>{num && <span>{num}</span>}{num && <span style={{ opacity: 0.6 }}>—</span>}<span>{badge}</span><span style={{ flexGrow: 1, height: 1, background: kc, opacity: 0.25, maxWidth: 240 }} /></p>}<h2 style={{ fontFamily: "'Poppins',sans-serif", fontWeight: 700, fontSize: "clamp(30px,4vw,42px)", color: light ? "#fff" : "#1D3557", margin: "0 0 12px", lineHeight: 1.15 }}>{title}</h2>{sub && <p style={{ fontFamily: "'Inter',sans-serif", fontSize: 16, color: light ? "rgba(255,255,255,0.75)" : "#3B6B8A", lineHeight: 1.6, margin: 0 }}>{sub}</p>}</div>; };
...
function DerivadorHAB({ landing: L, nav }) {
  ...
  return <div>
    <section style={{ background: "radial-gradient(ellipse at 28% 25%, #2B466B 0%, rgba(43,70,107,0) 55%), linear-gradient(150deg, #24395B 0%, #1D3557 50%, #15223A 100%)", padding: "130px 24px 80px", minHeight: "60vh", display: "flex", alignItems: "center" }}>
  ...
  return <div>
    <section style={{ background: "radial-gradient(ellipse at 28% 25%, #2B466B 0%, rgba(43,70,107,0) 55%), linear-gradient(150deg, #24395B 0%, #1D3557 50%, #15223A 100%)", position: "relative", overflow: "hidden" }}>
      ...
      <div style={{ maxWidth: 1140, margin: "0 auto", padding: "168px 40px 96px", position: "relative", zIndex: 1 }}>
        ...
        <p style={{ fontFamily: "'Inter',sans-serif", fontSize: 16, color: "rgba(241,243,245,0.7)", lineHeight: 1.65, maxWidth: 540, margin: "0 0 36px" }}>Resolvemos tus trámites registrales, municipales y habilitaciones comerciales en La Plata y toda la Provincia. Sin vueltas, sin moverte de tu casa.</p>
        <div style={{ display: "flex", gap: 12, flexWrap: "wrap" }}><Btn sm href={wl("Hola! Quiero hacer una consulta.")} style={{ fontSize: 12, padding: "9px 16px" }}><MessageCircle size={15} /> Consultar</Btn><Btn sm v="outlineW" onClick={() => document.getElementById("srv")?.scrollIntoView({ behavior: "smooth" })} style={{ fontSize: 12, padding: "9px 16px" }}>Ver servicios <ArrowDown size={14} /></Btn></div>
        </div>
      </div>
    </section>
    {/* Barra de confianza separada (estilo HAB): 4 métricas + gestor, ancho amplio 1400 */}
    <section style={{ background: "#132437", padding: "16px 24px", borderBottom: "1px solid rgba(255,255,255,0.06)" }}>
      <div style={{ maxWidth: 1240, margin: "0 auto", display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(200px, 1fr))", gap: 28, alignItems: "center" }}>
        {[{ icon: Star, n: "+350 reseñas", l: "5.0 ★ Google", href: "https://www.google.com/search?q=gestor+la+plata#mpd=~16740334214186789196/customers/reviews" }, { icon: BadgeCheck, n: "Mat. 10001", l: "Gestores PBA" }, { icon: Shield, n: "+2.000", l: "Trámites gestionados" }, { icon: MapPin, n: "Provincia de Bs. As.", l: "Cobertura" }].map((s, i) => { const I = s.icon; const inner = <><span style={{ width: 44, height: 44, borderRadius: "50%", background: "rgba(244,162,97,0.10)", border: "1px solid rgba(244,162,97,0.25)", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}><I size={20} color="#F4A261" /></span><span style={{ display: "flex", flexDirection: "column", lineHeight: 1.2 }}><span style={{ fontFamily: "'Poppins',sans-serif", fontWeight: 700, fontSize: 15, color: "#F1F3F5" }}>{s.n}</span><span style={{ fontFamily: "'Inter',sans-serif", fontSize: 11, color: "rgba(241,243,245,0.55)", marginTop: 2 }}>{s.l}</span></span></>; return s.href ? <a key={i} href={s.href} target="_blank" rel="noopener noreferrer" style={{ display: "flex", alignItems: "center", gap: 14, textDecoration: "none" }}>{inner}</a> : <div key={i} style={{ display: "flex", alignItems: "center", gap: 14 }}>{inner}</div>; })}
      </div>
    </section>
    <section id="srv" style={{ padding: "80px 40px", background: "#F1F3F5" }}><div style={{ maxWidth: 1140, margin: "0 auto" }}><ST num="01" badge="Servicios" title={<>Encontrá la solución <span style={{ color: "#F4A261" }}>para tu trámite</span></>} />
      <div style={{ display: "flex", flexWrap: "wrap", justifyContent: "center", gap: 20, marginTop: 24 }}>{MENU_SERVICES.map(s => { const I = s.icon; return <Card key={s.id} onClick={() => nav("block", s.id)} style={{ borderTop: `4px solid #1D3557`, padding: 0, overflow: "hidden", flex: "0 0 calc((100% - 20px) / 2)", minWidth: 280, maxWidth: 520 }}><div style={{ padding: "22px 22px 0" }}><div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 12 }}><div style={{ width: 46, height: 46, borderRadius: 11, background: "#1D355712", display: "flex", alignItems: "center", justifyContent: "center" }}><I size={22} color="#1D3557" /></div><Bdg c={scopeColor(s.scope)}>{scopeLabel(s.scope)}</Bdg></div><h3 style={{ fontFamily: "'Poppins',sans-serif", fontWeight: 700, fontSize: 17, color: "#1D3557", margin: "0 0 6px" }}>{s.title}</h3><p style={{ fontFamily: "'Inter',sans-serif", fontSize: 13.5, color: "#3B6B8A", lineHeight: 1.55, margin: 0 }}>{s.desc}</p></div>
    ...
    <section style={{ padding: "80px 40px", background: "#fff" }}><div style={{ maxWidth: 1140, margin: "0 auto" }}><ST num="02" badge="Confianza y transparencia" title={<>¿Por qué contratarnos<br /><span style={{ color: "#F4A261" }}>sin conocernos?</span></>} />
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(320px, 1fr))", gap: 20 , marginTop: 24 }}>{[
        ...
        { icon: Star, t: "+350 reseñas 5 estrellas en Google", d: "Podés leerlas antes de iniciar la gestión." },
        { icon: Smartphone, t: "Documentación 100% oficial", d: "Todos los documentos son emitidos directamente por los organismos. Tienen firma digital y código QR." },
        ...
        { icon: Shield, t: "Precio final", d: "El presupuesto incluye honorarios y timbrados. No hay costos ocultos." }
      ].map((d, i) => { const I = d.icon; return <div key={i} style={{ background: "#F8F9FA", borderRadius: 14, padding: 28, border: "1px solid #E9ECEF", display: "flex", gap: 16, alignItems: "flex-start" }}><div style={{ width: 44, height: 44, borderRadius: 10, background: "rgba(29,53,87,0.10)", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}><I size={22} color="#1D3557" /></div><div><h3 style={{ fontFamily: "'Poppins',sans-serif", fontWeight: 700, fontSize: 15, color: "#1D3557", margin: "0 0 8px" }}>{d.t}</h3><p style={{ fontFamily: "'Inter',sans-serif", fontSize: 14, color: "#3B6B8A", lineHeight: 1.6, margin: 0 }}>{d.d}</p></div></div>; })}</div>
    ...
    <section style={{ padding: "80px 40px", background: "#F1F3F5" }}><div style={{ maxWidth: 1140, margin: "0 auto" }}><ST num="03" badge="Nuestro compromiso" title={<>Garantías escritas,<br /><span style={{ color: "#F4A261" }}>no promesas vacías</span></>} />
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(260px, 1fr))", gap: 20, maxWidth: 1000, margin: "0 auto" , marginTop: 24 }}>{[
        { n: "01", c: "#1D3557", t: "Si el error es nuestro, lo corregimos sin costo", d: "Antes de presentar, revisamos cada dato y requisito. Si hay un error imputable a nuestra gestión, lo corregimos y reingresamos sin costo adicional." },
        ...
        { n: "03", c: "#1D3557", t: "Documentación oficial", d: "Todo lo que entregamos es documentación emitida por el organismo correspondiente, con firma digital y/o código QR verificable." }
    ...
    <section style={{ padding: "80px 40px", background: "#fff" }}><div style={{ maxWidth: 1140, margin: "0 auto" }}><ST num="04" badge="Testimonios" title={<>Lo que dicen quienes <span style={{ color: "#F4A261" }}>ya confiaron en nosotros</span></>} /><div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(280px, 1fr))", gap: 24 }}>{REVIEWS.map((r, i) => <div key={i} style={{ background: "#F8F9FA", borderRadius: 14, padding: 32, position: "relative", border: "1px solid #E9ECEF" }}><div style={{ position: "absolute", top: 16, right: 20, fontFamily: "Georgia,serif", fontSize: 72, color: "#3B6B8A", opacity: 0.08, lineHeight: 1 }}>"</div><div style={{ display: "flex", gap: 2, marginBottom: 16 }}>{[...Array(r.stars)].map((_, j) => <Star key={j} size={16} fill="#F4A261" color="#F4A261" />)}</div><p style={{ fontFamily: "'Inter',sans-serif", fontSize: 15, color: "#212529", lineHeight: 1.7, margin: "0 0 20px", position: "relative", zIndex: 1 }}>{r.text}</p><div style={{ display: "flex", alignItems: "center", gap: 12 }}><div style={{ width: 40, height: 40, borderRadius: "50%", background: "#1D3557", display: "flex", alignItems: "center", justifyContent: "center" }}><span style={{ fontFamily: "'Poppins',sans-serif", fontWeight: 700, fontSize: 16, color: "#F1F3F5" }}>{r.author[0]}</span></div><div><div style={{ fontFamily: "'Poppins',sans-serif", fontWeight: 600, fontSize: 14, color: "#1D3557" }}>{r.author}</div><div style={{ fontFamily: "'Inter',sans-serif", fontSize: 12, color: "#3B6B8A" }}>{r.rubro}</div></div></div></div>)}</div>
          <div style={{ textAlign: "center", marginTop: 32 }}>
            <Btn sm href={GOOGLE_REVIEWS_URL} v="outline"><Star size={14} /> Ver las +350 reseñas en Google</Btn>
          </div>
        </div></section>
    <section style={{ padding: "80px 40px", background: "radial-gradient(ellipse at 28% 25%, #2B466B 0%, rgba(43,70,107,0) 55%), linear-gradient(150deg, #24395B 0%, #1D3557 50%, #15223A 100%)", position: "relative", overflow: "hidden" }}><div style={{ position: "absolute", top: 0, right: 0, width: "40%", height: "100%", background: "linear-gradient(135deg, rgba(69,123,157,0.1), transparent)" }} /><div style={{ maxWidth: 1140, margin: "0 auto", position: "relative", zIndex: 1 }}><ST num="05" badge="Diferenciales" title={<>¿Por qué <span style={{ color: "#F4A261" }}>elegirnos?</span></>} light /><div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(260px, 1fr))", gap: 20 }}>{[{ icon: MapPin, t: "Enfoque 100% Local", d: "Dominamos cada dependencia de La Plata y PBA." }, { icon: Zap, t: "Agilidad Real", d: "Presencia diaria en mesas de entrada. Respuesta rápida garantizada." }, { icon: Shield, t: "Rigurosidad Técnica", d: "Revisamos todo antes de presentar. Cero rechazos." }, { icon: Smartphone, t: "100% Digital", d: "Todo por WhatsApp. Seguimiento en tiempo real." }].map((d, i) => { const I = d.icon; return <div key={i} style={{ padding: 32, borderRadius: 14, background: "rgba(255,255,255,0.04)", border: "1px solid rgba(255,255,255,0.08)" }}><div style={{ width: 48, height: 48, borderRadius: 10, background: "rgba(244,162,97,0.12)", display: "flex", alignItems: "center", justifyContent: "center", marginBottom: 20 }}><I size={24} color="#F4A261" /></div><h3 style={{ fontFamily: "'Poppins',sans-serif", fontWeight: 600, fontSize: 18, color: "#F1F3F5", margin: "0 0 8px" }}>{d.t}</h3><p style={{ fontFamily: "'Inter',sans-serif", fontSize: 14, color: "rgba(241,243,245,0.6)", lineHeight: 1.7, margin: 0 }}>{d.d}</p></div>; })}</div></div></section>
    <section style={{ padding: "80px 40px", background: "#F1F3F5" }}><div style={{ maxWidth: 1000, margin: "0 auto" }}><ST num="06" badge="Proceso" title={<>Cómo <span style={{ color: "#F4A261" }}>Trabajamos</span></>} sub="4 pasos. Sin moverte de tu casa." /><div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(220px, 1fr))" }}>{[{ n: "01", t: "Consulta", d: "Escribinos por WhatsApp. Te pasamos presupuesto sin cargo.", icon: Send, c: "#1D3557" }, { n: "02", t: "Confirmación", d: "Abonás por transferencia o Mercado Pago.", icon: CircleDollarSign, c: "#3B6B8A" }, { n: "03", t: "Gestión", d: "Ingresamos y hacemos seguimiento activo.", icon: Search, c: "#1D3557" }, { n: "04", t: "Entrega", d: "Vía WhatsApp o Correo electrónico.", icon: BadgeCheck, c: "#3B6B8A" }].map((s, i) => { const I = s.icon; return <div key={i} style={{ textAlign: "center", padding: "20px 24px" }}><div style={{ width: 70, height: 70, borderRadius: "50%", background: s.c, display: "flex", alignItems: "center", justifyContent: "center", margin: "0 auto 20px", boxShadow: `0 8px 24px ${s.c}33`, position: "relative" }}><I size={28} color="#fff" /><div style={{ position: "absolute", top: -6, right: -6, width: 26, height: 26, borderRadius: "50%", background: "#fff", display: "flex", alignItems: "center", justifyContent: "center", fontFamily: "'Poppins',sans-serif", fontWeight: 700, fontSize: 11, color: s.c, boxShadow: "0 2px 8px rgba(0,0,0,0.1)" }}>{s.n}</div></div><h3 style={{ fontFamily: "'Poppins',sans-serif", fontWeight: 700, fontSize: 17, color: "#1D3557", margin: "0 0 8px" }}>{s.t}</h3><p style={{ fontFamily: "'Inter',sans-serif", fontSize: 14, color: "#3B6B8A", lineHeight: 1.6, margin: 0 }}>{s.d}</p></div>; })}</div><div style={{ textAlign: "center", marginTop: 40 }}><Btn href={wl("Hola! Quiero iniciar un trámite.")} v="primary"><MessageCircle size={18} /> Empezá tu trámite ahora</Btn></div></div></section>
    <section style={{ padding: "80px 40px", background: "#fff" }}><div style={{ maxWidth: 800, margin: "0 auto" }}><ST num="07" badge="FAQ" title={<>Preguntas <span style={{ color: "#F4A261" }}>Frecuentes</span></>} />{FAQ.map((f, i) => <FI key={i} q={f.q} a={f.a} />)}<div style={{ textAlign: "center", marginTop: 32 }}><Btn sm href={wl("Hola! Tengo una duda.")} v="outline"><MessageCircle size={14} /> Consultá con un Asesor</Btn></div></div></section>
    <section style={{ padding: "60px 24px", background: "radial-gradient(ellipse at 28% 25%, #2B466B 0%, rgba(43,70,107,0) 55%), linear-gradient(150deg, #24395B 0%, #1D3557 50%, #15223A 100%)" }}><div style={{ maxWidth: 800, margin: "0 auto", textAlign: "center" }}><img src="/isotipo-glp.png" alt="Gestor La Plata" width="120" height="120" style={{ width: 120, height: 120, objectFit: "contain", display: "block", margin: "0 auto 10px" }} /><h2 style={{ fontFamily: "'Poppins',sans-serif", fontWeight: 700, fontSize: "clamp(24px,3.5vw,34px)", color: "#F1F3F5", margin: "0 0 12px" }}>¿Listo para <span style={{ color: "#F4A261" }}>resolver</span> tu trámite?</h2><p style={{ fontFamily: "'Inter',sans-serif", fontSize: 16, color: "rgba(241,243,245,0.65)", margin: "0 0 32px" }}>Presupuesto sin cargo. Respondemos en minutos.</p><div style={{ display: "flex", gap: 12, justifyContent: "center", flexWrap: "wrap" }}><Btn sm href={wl("Hola, necesito habilitar un local en La Plata.\n¿Me podés asesorar sobre qué necesito?")} style={{ fontSize: 12, padding: "10px 18px" }}><MessageCircle size={15} /> Escribinos por WhatsApp →</Btn></div></div></section>
  </div>;
}
...
function BlockPage({ service: s, nav }) {
  ...
  return <div>
    ...
    <section style={{ padding: "60px 24px", background: "#F1F3F5" }}><div style={{ maxWidth: 1200, margin: "0 auto" }}><ST badge="Servicios" title="Elegí el que necesitás" />
      <div className="grid-2" style={{ display: "grid", gridTemplateColumns: "repeat(2, minmax(0, 1fr))", gap: 20 }}>{s.subs.map(sub => <Card key={sub.id} onClick={() => nav("sub", s.id, sub.id)} style={{ borderLeft: `4px solid ${s.color}` }}>
...
function SubPage({ service: s, sub, nav }) {
  ...
  return <div>
    ...
    <section style={{ padding: "60px 24px", background: "#F1F3F5" }}><div style={{ maxWidth: 1200, margin: "0 auto" }}><ST badge="Trámites" title="Identificá tu situación" />
      <div className="grid-2" style={{ display: "grid", gridTemplateColumns: "repeat(2, minmax(0, 1fr))", gap: 20 }}>{sub.landings.map(l => <Card key={l.id} onClick={() => nav("landing", s.id, sub.id, l.id)} style={{ borderLeft: "4px solid #F4A261" }}>
...
export default function App() {
  ...
  // Al scrollear hacia abajo ocultamos la topbar fina; el header principal queda sticky arriba.
  ...
  useEffect(() => {
    ...
    if (!document.querySelector('link[rel="icon"]')) {
      const fav = document.createElement("link"); fav.rel = "icon"; fav.type = "image/png"; fav.href = "/favicon-glp.png"; document.head.appendChild(fav);
    }
  }, [r, cs, csub, cl]);
  ...
  return <div style={{ fontFamily: "'Inter',sans-serif", background: "#F1F3F5", minHeight: "100vh", color: "#212529", overflowX: "hidden", maxWidth: "100vw" }}>
    ...
    <div className="topbar" style={{ position: "fixed", top: 0, left: 0, right: 0, zIndex: 1000, background: "#16233A", height: scrolled ? 0 : 32, opacity: scrolled ? 0 : 1, overflow: "hidden", pointerEvents: scrolled ? "none" : "auto", display: "flex", alignItems: "center", transition: "height 0.25s ease, opacity 0.25s ease" }}>
      <div style={{ maxWidth: 1400, margin: "0 auto", padding: "0 32px", display: "flex", alignItems: "center", justifyContent: "flex-start", gap: 10, fontFamily: "'Inter',sans-serif", fontSize: 12, color: "#F1F3F5", width: "100%" }}>
        <a href={GOOGLE_REVIEWS_URL} target="_blank" rel="noopener noreferrer" style={{ display: "inline-flex", alignItems: "center", gap: 6, color: "#F1F3F5", textDecoration: "none" }}><Star size={12} fill="#F1F3F5" color="#F1F3F5" /> <strong style={{ fontWeight: 600 }}>5.0 Google</strong></a>
        <span style={{ color: "rgba(255,255,255,0.5)" }}>·</span>
        <a href={GOOGLE_REVIEWS_URL} target="_blank" rel="noopener noreferrer" style={{ display: "inline-flex", alignItems: "center", color: "#F1F3F5", textDecoration: "none" }}>+350 reseñas</a>
        <span style={{ color: "rgba(255,255,255,0.5)" }}>·</span>
        <span style={{ display: "inline-flex", alignItems: "center" }}>Mat. 10001</span>
      </div>
    </div>
    <nav className="mainnav" style={{ position: "fixed", top: scrolled ? 0 : 32, left: 0, right: 0, zIndex: 999, background: "#1D3557", backdropFilter: "blur(12px)", borderBottom: "1px solid rgba(255,255,255,0.06)", transition: "top 0.25s ease"}}>
      <div style={{ maxWidth: 1400, margin: "0 auto", padding: "0 32px", height: 64, display: "flex", alignItems: "center", justifyContent: "space-between" }}>
        <div onClick={() => nav("home")} role="button" aria-label="Ir al inicio - Gestor La Plata" style={{ cursor: "pointer", display: "flex", alignItems: "center", gap: 10, marginRight: 32, flexShrink: 0 }}><img src="/isotipo-glp.png" alt="Gestor La Plata" width="52" height="52" style={{ width: 52, height: 52, objectFit: "contain", display: "block" }} /><span style={{ fontFamily: "'Poppins',sans-serif", fontWeight: 600, fontSize: 15, letterSpacing: "0.08em", color: "#F1F3F5", whiteSpace: "nowrap" }}>GESTOR <span style={{ color: "#F4A261", fontWeight: 600 }}>LA PLATA</span></span></div>
        <div className="dn" style={{ display: "flex", alignItems: "center", gap: 2 }}><NB o={() => nav("home")} a={r.p === "home"}>Inicio</NB>{MENU_SERVICES.map(s => <NB key={s.id} o={() => nav("block", s.id)} a={r.b === s.id}>{s.title.replace("Servicios ", "").replace("Comerciales", "")}</NB>)}<NB o={() => nav("about")} a={r.p === "about"}>Nosotros</NB><Btn sm href={wl("Hola!")} style={{ marginLeft: 8, fontSize: 10, fontWeight: 700, padding: "8px 14px", letterSpacing: "0.08em", gap: 5 }}><MessageCircle size={13} /> Contacto</Btn></div>
      ...
      {mo && <div className="mm" style={{ background: "#1D3557", padding: "16px 24px 24px", borderTop: "1px solid rgba(255,255,255,0.06)" }}><NB o={() => nav("home")} m>Inicio</NB>{MENU_SERVICES.map(s => <NB key={s.id} o={() => nav("block", s.id)} m>{s.title}</NB>)}<NB o={() => nav("about")} m>Nosotros</NB><div style={{ marginTop: 12 }}><Btn sm href={wl("Hola!")}><MessageCircle size={14} /> Contacto</Btn></div></div>}
    </nav>
    <main>
    ...
    <footer style={{ background: "#132437", color: "rgba(241,243,245,0.7)" }}><div style={{ display: "grid", gridTemplateColumns: "1.6fr 1fr 1fr 1.2fr", gap: 40, maxWidth: 1400, margin: "0 auto", padding: "64px 40px 48px" }} className="glp-footer-grid"><div><div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 16 }}><img src="/isotipo-glp.png" alt="" aria-hidden="true" width="36" height="36" style={{ width: 36, height: 36, objectFit: "contain", display: "block", flexShrink: 0 }} /><span style={{ display: "flex", flexDirection: "column", lineHeight: 1.1, fontFamily: "'Poppins',sans-serif", fontWeight: 700, fontSize: 17 }}><span style={{ color: "#F1F3F5" }}>Gestor</span><span style={{ color: "#F4A261" }}>La Plata</span></span></div><p style={{ fontSize: 13, lineHeight: 1.6, marginBottom: 12, color: "rgba(241,243,245,0.6)", maxWidth: 320 }}>Tu tiempo vale. Dejá la burocracia en nuestras manos. Gestoría integral en La Plata y Provincia de Buenos Aires.</p><p style={{ fontSize: 13, fontWeight: 600, color: "#F4A261", marginBottom: 12 }}>Atención personalizada</p><p style={{ fontSize: 12, color: "rgba(241,243,245,0.45)", lineHeight: 1.5, margin: 0 }}>Santiago A. Ippolito · Gestor · Mat. 10001 PBA</p><p style={{ fontFamily: "'Inter',sans-serif", fontWeight: 600, fontSize: 12, color: "#F1F3F5", textTransform: "uppercase", letterSpacing: "0.1em", margin: "24px 0 16px" }}>Nuestras webs</p><ul style={{ display: "flex", flexDirection: "column", gap: 9, listStyle: "none", margin: 0, padding: 0 }}><li><a href="https://www.gestorlaplata.com" target="_blank" rel="noopener noreferrer" className="footer-link" style={{ display: "inline-flex", alignItems: "center", gap: 4, fontSize: 13, color: "rgba(241,243,245,0.6)", textDecoration: "none" }}>gestorlaplata.com <ArrowUpRight size={12} strokeWidth={2.5} aria-hidden="true" /></a></li><li><a href="https://www.habilitacioneslaplata.com" target="_blank" rel="noopener noreferrer" className="footer-link" style={{ display: "inline-flex", alignItems: "center", gap: 4, fontSize: 13, color: "rgba(241,243,245,0.6)", textDecoration: "none" }}>habilitacioneslaplata.com <ArrowUpRight size={12} strokeWidth={2.5} aria-hidden="true" /></a></li><li><a href="https://www.tramitesregistralespba.com" target="_blank" rel="noopener noreferrer" className="footer-link" style={{ display: "inline-flex", alignItems: "center", gap: 4, fontSize: 13, color: "rgba(241,243,245,0.6)", textDecoration: "none" }}>tramitesregistralespba.com <ArrowUpRight size={12} strokeWidth={2.5} aria-hidden="true" /></a></li></ul></div><div><p style={{ fontFamily: "'Inter',sans-serif", fontWeight: 600, fontSize: 12, color: "#F1F3F5", textTransform: "uppercase", letterSpacing: "0.1em", marginBottom: 16 }}>Servicios</p><ul style={{ display: "flex", flexDirection: "column", gap: 9, listStyle: "none", margin: 0, padding: 0 }}>{MENU_SERVICES.map(sv => <li key={sv.id}><a onClick={() => nav("block", sv.id)} className="footer-link" style={{ display: "inline-flex", alignItems: "center", fontSize: 13, color: "rgba(241,243,245,0.6)", textDecoration: "none", cursor: "pointer" }}>{sv.title}</a></li>)}</ul></div><div><p style={{ fontFamily: "'Inter',sans-serif", fontWeight: 600, fontSize: 12, color: "#F1F3F5", textTransform: "uppercase", letterSpacing: "0.1em", marginBottom: 16 }}>Marcas del grupo</p><ul style={{ display: "flex", flexDirection: "column", gap: 9, listStyle: "none", margin: 0, padding: 0 }}><li><a href="https://www.habilitacioneslaplata.com" target="_blank" rel="noopener noreferrer" className="footer-link" style={{ display: "inline-flex", alignItems: "center", gap: 7, fontSize: 13, color: "rgba(241,243,245,0.6)", textDecoration: "none" }}><Store size={15} style={{ color: "#F4A261", flexShrink: 0 }} aria-hidden="true" /> Habilitaciones La Plata</a></li><li><a href="https://www.tramitesregistralespba.com" target="_blank" rel="noopener noreferrer" className="footer-link" style={{ display: "inline-flex", alignItems: "center", gap: 7, fontSize: 13, color: "rgba(241,243,245,0.6)", textDecoration: "none" }}><FileText size={15} style={{ color: "#F4A261", flexShrink: 0 }} aria-hidden="true" /> Trámites Registrales PBA</a></li><li style={{ fontSize: 12, color: "rgba(241,243,245,0.4)", lineHeight: 1.5, marginTop: 4 }}>Unidades especializadas de Gestor La Plata.</li></ul></div><div><p style={{ fontFamily: "'Inter',sans-serif", fontWeight: 600, fontSize: 12, color: "#F1F3F5", textTransform: "uppercase", letterSpacing: "0.1em", marginBottom: 16 }}>Contacto</p><ul style={{ display: "flex", flexDirection: "column", gap: 9, listStyle: "none", margin: 0, padding: 0 }}><li><a href="https://wa.me/5492214886197" target="_blank" rel="noopener noreferrer" className="footer-link" style={{ display: "inline-flex", alignItems: "center", gap: 7, fontSize: 13, color: "rgba(241,243,245,0.6)", textDecoration: "none" }}><MessageCircle size={15} style={{ color: "#F4A261", flexShrink: 0 }} aria-hidden="true" /> 221-488-6197 (WhatsApp)</a></li><li><a href="mailto:contacto@gestorlaplata.com" className="footer-link" style={{ display: "inline-flex", alignItems: "center", gap: 7, fontSize: 13, color: "rgba(241,243,245,0.6)", textDecoration: "none" }}><Mail size={15} style={{ color: "#F4A261", flexShrink: 0 }} aria-hidden="true" /> contacto@gestorlaplata.com</a></li><li><a href="https://www.google.com/maps/search/?api=1&query=Calle%2049%201295%2C%20La%20Plata%2C%20Buenos%20Aires" target="_blank" rel="noopener noreferrer" className="footer-link" style={{ display: "inline-flex", alignItems: "center", gap: 7, fontSize: 13, color: "rgba(241,243,245,0.6)", textDecoration: "none" }}><MapPin size={15} style={{ color: "#F4A261", flexShrink: 0 }} aria-hidden="true" /> Calle 49 N° 1295, La Plata</a></li><li><a href="https://www.instagram.com/gestorlaplata" target="_blank" rel="noopener noreferrer" className="footer-link" style={{ display: "inline-flex", alignItems: "center", gap: 7, fontSize: 13, color: "rgba(241,243,245,0.6)", textDecoration: "none" }}><svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="#F4A261" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" style={{ flexShrink: 0 }} aria-hidden="true"><rect x="2" y="2" width="20" height="20" rx="5" ry="5"/><path d="M16 11.37A4 4 0 1 1 12.63 8 4 4 0 0 1 16 11.37z"/><line x1="17.5" y1="6.5" x2="17.51" y2="6.5"/></svg> @gestorlaplata</a></li><li><a href={GOOGLE_REVIEWS_URL} target="_blank" rel="noopener noreferrer" className="footer-link" style={{ display: "inline-flex", alignItems: "center", gap: 7, fontSize: 13, color: "rgba(241,243,245,0.6)", textDecoration: "none" }}><Star size={15} fill="#F4A261" style={{ color: "#F4A261", flexShrink: 0 }} aria-hidden="true" /> +350 reseñas en Google</a></li><li style={{ display: "inline-flex", alignItems: "flex-start", gap: 7, fontSize: 13, color: "rgba(241,243,245,0.6)" }}><Clock size={15} style={{ color: "#F4A261", flexShrink: 0, marginTop: 2 }} aria-hidden="true" /><span>Lunes a viernes de 8 a 18 hs<br />Sábados de 9 a 12 hs</span></li></ul><div style={{ marginTop: 18, borderRadius: 12, overflow: "hidden", border: "1px solid rgba(255,255,255,0.12)", lineHeight: 0 }}><LazyMap /></div></div></div><div style={{ borderTop: "1px solid rgba(255,255,255,0.08)", padding: "22px 0", textAlign: "center" }}><div style={{ maxWidth: 1400, margin: "0 auto", padding: "0 40px" }}><nav aria-label="Enlaces legales" style={{ display: "flex", flexWrap: "wrap", justifyContent: "center", alignItems: "center", gap: "6px 10px", marginBottom: 14 }}><button onClick={() => nav("about")} style={{ background: "none", border: "none", fontFamily: "'Inter',sans-serif", fontSize: 12, color: "rgba(241,243,245,0.6)", cursor: "pointer", padding: 0 }}>Quiénes somos</button><span aria-hidden="true" style={{ color: "rgba(255,255,255,0.25)" }}>·</span><button onClick={() => nav("privacy")} style={{ background: "none", border: "none", fontFamily: "'Inter',sans-serif", fontSize: 12, color: "rgba(241,243,245,0.6)", cursor: "pointer", padding: 0 }}>Privacidad</button><span aria-hidden="true" style={{ color: "rgba(255,255,255,0.25)" }}>·</span><button onClick={() => nav("cookies")} style={{ background: "none", border: "none", fontFamily: "'Inter',sans-serif", fontSize: 12, color: "rgba(241,243,245,0.6)", cursor: "pointer", padding: 0 }}>Cookies</button></nav><p style={{ fontFamily: "'Inter',sans-serif", fontSize: 12, color: "rgba(241,243,245,0.4)", margin: 0 }}>© 2026 Gestor La Plata · Santiago A. Ippolito · Mat. 10001 PBA · Colegio de Gestores de la Provincia de Buenos Aires</p><p style={{ fontFamily: "'Inter',sans-serif", fontSize: 11, color: "rgba(241,243,245,0.3)", margin: "6px 0 0" }}>Este sitio es informativo. La normativa puede actualizarse. Consultanos para verificar los requisitos vigentes.</p></div></div></footer>
    <WAFloat />
...
function NB({ children, o, a, m }) { return <button onClick={o} className="navbtn" style={{ background: a ? "rgba(69,123,157,0.15)" : "none", border: "none", color: a ? "#F1F3F5" : "rgba(241,243,245,0.6)", fontFamily: "'Inter',sans-serif", fontWeight: 600, fontSize: m ? 14 : 10, letterSpacing: m ? "0" : "0.07em", textTransform: m ? "none" : "uppercase", padding: m ? "10px 0" : "6px 9px", borderRadius: 4, cursor: "pointer", display: m ? "block" : "inline-flex", width: m ? "100%" : "auto", textAlign: m ? "left" : "center", whiteSpace: "nowrap", transition: "color 0.2s" }}>{children}</button>; }
