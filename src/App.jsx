...
function AboutPage({ nav }) {
  ...
  return <div>
    ...
    {/* Números */}
    <section style={{ padding: "60px 24px", background: "radial-gradient(ellipse at 28% 25%, #2B466B 0%, rgba(43,70,107,0) 55%), linear-gradient(150deg, #24395B 0%, #1D3557 50%, #15223A 100%)" }}><div style={{ maxWidth: 1200, margin: "0 auto" }}><div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(200px, 1fr))", gap: 20 }}>{[{ n: "+2.000", l: "Trámites gestionados", icon: BadgeCheck }, { n: "+340", l: "Reseñas 5 estrellas", icon: Star }, { n: "5", l: "Áreas de gestión", icon: FileText }, { n: "PBA", l: "Toda la Provincia", icon: Globe }].map((s, i) => { const I = s.icon; return <div key={i} style={{ textAlign: "center", padding: 28, borderRadius: 14, background: "rgba(255,255,255,0.04)", border: "1px solid rgba(255,255,255,0.08)" }}><div style={{ width: 48, height: 48, borderRadius: 10, background: "rgba(244,162,97,0.12)", display: "flex", alignItems: "center", justifyContent: "center", margin: "0 auto 16px" }}><I size={24} color="#F4A261" /></div><div style={{ fontFamily: "'Poppins',sans-serif", fontWeight: 700, fontSize: 32, color: "#F1F3F5", lineHeight: 1 }}>{s.n}</div><div style={{ fontFamily: "'Inter',sans-serif", fontSize: 14, color: "rgba(241,243,245,0.6)", marginTop: 8 }}>{s.l}</div></div>; })}</div></div></section>
    {/* Organismos */}
