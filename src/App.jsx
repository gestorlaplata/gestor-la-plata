...
function HomePage({ nav }) {
  ...
  return <div>
    ...
    <section id="srv" style={{ padding: "80px 40px", background: "#F1F3F5" }}><div style={{ maxWidth: 1140, margin: "0 auto" }}><ST num="01" badge="Servicios" title={<>Encontrá la solución <span style={{ color: "#F4A261" }}>para tu trámite</span></>} />
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(350px, 1fr))", gap: 20 }}>{SERVICES.map(s => { const I = s.icon; return <Card key={s.id} onClick={() => nav("block", s.id)} style={{ borderTop: `4px solid ${s.color}`, padding: 0, overflow: "hidden" }}><div style={{ padding: "22px 22px 0" }}><div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 12 }}><div style={{ width: 46, height: 46, borderRadius: 11, background: s.color + "12", display: "flex", alignItems: "center", justifyContent: "center" }}><I size={22} color={s.color} /></div><Bdg c={scopeColor(s.scope)}>{scopeLabel(s.scope)}</Bdg></div><h3 style={{ fontFamily: "'Poppins',sans-serif", fontWeight: 700, fontSize: 17, color: "#1D3557", margin: "0 0 6px" }}>{s.title}</h3><p style={{ fontFamily: "'Inter',sans-serif", fontSize: 13.5, color: "#3B6B8A", lineHeight: 1.55, margin: 0 }}>{s.desc}</p></div>
        <div style={{ display: "flex", flexWrap: "wrap", gap: 6, padding: "14px 22px", margin: "14px 0 0", borderTop: "1px solid #F1F3F5", background: "#FAFBFC" }}>{s.subs.slice(0, 3).map(sub => <span key={sub.id} style={{ fontFamily: "'Inter',sans-serif", fontSize: 12, color: "#3B6B8A", background: "#EDF2F7", padding: "3px 10px", borderRadius: 20 }}>{sub.title}</span>)}{s.subs.length > 3 && <span style={{ fontFamily: "'Inter',sans-serif", fontSize: 12, color: "#F4A261", fontWeight: 600, padding: "3px 10px" }}>+{s.subs.length - 3} más</span>}</div>
